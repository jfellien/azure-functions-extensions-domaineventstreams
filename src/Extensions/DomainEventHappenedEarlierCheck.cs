...
namespace devCrowd.CustomBindings.EventSourcing.Extensions
{
        ...
        where TEarlier : IDomainEvent 
    {
        ...
        {
            var indexOfEarlierEvent = _sourceStream.ToList().IndexOf(_earlierEvent);

            var laterEvent = _sourceStream.OfType<TLater>().LastOrDefault(filterExpression);
            ...
            var indexOfLaterEvent = _sourceStream.ToList().IndexOf(laterEvent);
