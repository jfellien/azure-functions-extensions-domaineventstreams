...
public class SqlServerDomainEventStreamStorage : IReadAndWriteDomainEvents
{
    ...
    {
        string insertQuery = InsertStatementBuilder.GetStatement(_tableName, entity, entityId);
            
        await using SqlConnection connection = new SqlConnection(_connectionString);
        await using SqlCommand command = new SqlCommand(insertQuery, connection);

        string serializedDomainEvent = JsonConvert.SerializeObject(domainEvent);
        long nextSequenceNumber = await NextSequenceNumber(context, entity, entityId);
    ...
    {
        string eventType = reader.GetString(SqlServerDomainEventStreamStorageColumnNames.EventFullName);
        ...
        string domainEventAsString = reader.GetString(SqlServerDomainEventStreamStorageColumnNames.PayLoad);
        IDomainEvent domainEventInstance =
            JsonConvert.DeserializeObject(domainEventAsString, domainEventType, _defaultSerializerSettings) as IDomainEvent;


        long sequenceNumber = reader.GetInt64(SqlServerDomainEventStreamStorageColumnNames.SequenceNumber);

        SequencedDomainEvent sequencedDomainEvent = new SequencedDomainEvent(sequenceNumber, domainEventInstance);
    ...
    {
        string query = BuildQueryStringForLastSequenceNumber(context, entity, entityId);
        ...
        try
        {
            ...
            long lastSequenceNumber = scalarResult is DBNull ? 0 : Convert.ToInt64(scalarResult);
